...
BitmapDecoder::BitmapDecoder(std::string filename) {
    ...
    BitmapDecoder::skipBytes(bmp_file, 8);

    // getting image size in bytes
    image_size = BitmapDecoder::getWord(bmp_file);
    ...
    BitmapDecoder::skipBytes(bmp_file, 16);
...
void BitmapDecoder::createRgbMatrix(std::ifstream& bmp_file) {
    ...
    for (uint32_t image_byte_idx = 0; image_byte_idx < image_size; ++image_byte_idx) {
        
        // 0 = red, 1 = green, 2 = blue
        std::uint8_t color_layer = 2 - (image_byte_idx % 3);

        // image read goes from left to right, 3 bytes per pixel
        std::uint32_t pixel_width = (image_byte_idx / 3) % width;

        // image read goes bottom-up
        std::uint32_t pixel_height = height-1 - (image_byte_idx / 3 / width);

        file_byte = bmp_file.get();

        rgb_img_matrix[pixel_height][pixel_width][color_layer] = file_byte;
    }
}
