  ...
  */
  ...
  for (std::uint32_t rowIdx = 0; rowIdx < originalHeight; ++rowIdx) {
    for (std::uint32_t colIdx = 0; colIdx < originalWidth; ++colIdx) {
      ...
      RgbVal rgbVal = {0, 0, 0};
      rgbVal.r = static_cast<std::uint8_t>(std::max(static_cast<double>(0), std::round(y + (1.402 * (cr - 128)))));
      rgbVal.g = static_cast<std::uint8_t>(std::max(static_cast<double>(0), std::round(y + (-0.34414 * (cb - 128)) + (-0.71414 * (cr - 128)))));
      rgbVal.b = static_cast<std::uint8_t>(std::max(static_cast<double>(0), std::round(y + (1.772 * (cb - 128)))));
